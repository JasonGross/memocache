...
def _json_default(
    ...
    elif torch is not None and (
        isinstance(thing, torch.device) or isinstance(thing, torch.dtype)
    ):
        return str(thing)
    ...
    elif (torch is not None and isinstance(thing, torch.Tensor)) or (
        numpy is not None and isinstance(thing, numpy.ndarray)
    ):
        return _json_dumps(
            thing.tolist(),
            exclude_filter=exclude_filter,
    ...
    elif HookedTransformer is not None and isinstance(thing, HookedTransformer):
        return _json_dumps(
